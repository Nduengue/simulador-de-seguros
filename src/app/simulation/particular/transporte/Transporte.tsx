...
interface IApiListData {
    ...
    franchise: IGetIdAndNameMapperResponse[];
    claim_histories: IGetIdAndNameMapperResponse[];
}
...
export default function Transporte() {
    ...
    const [claimHistoryId, setClaimHistoryId] = useState<string>("");
        ...
        if (insurance_id && policy_type_id && insurance_type_id) {
            getListData()
                .then((response: GoodsTransportedType) => {
                    const { merchandises, ways, countries, states, from_tos, conditions, coverages, packaging, franchise, claim_histories } = response;
                    ...
                    setApiListDataResponse({
                        ...
                        franchise: GetIdAndNameMapper(franchise), //GetIdAndNameMapper
                        claim_histories: GetIdAndNameMapper(claim_histories), //GetIdAndNameMapper
                    });
        ...
        },
        {
            ...
            content: (
                <StepTwo
                    ...
                    setClaimHistoryId={setClaimHistoryId}
                    clainHistories={apiListDataResponse.claim_histories}
                />
            ),
        },
        {
        ...
        },
        {
            ...
            content: (
                <StepFour
                    ...
                    setIsCustomOrigin={setIsCustomOrigin}
                    predefinedLocations={predefinedLocations}
                    franchise={franchise}
                    setFranchise={setFranchise}
                    franchises={apiListDataResponse.franchise}
                />
            ),
        },
        {
    ...
    async function handleSubmitFn() {
        const dataBody = {
            ...
            claim_history_id: Number(claimHistoryId),
        ...
        try {
            const res = await fetch(`${API_LOCATION_2}/simulator/mt/save`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
...
function StepTwo({
    ...
    setClaimHistoryId: (e: string) => void;
    clainHistories: IGetIdAndNameMapperResponse[];
}) {
    return (
        <div className="pt-4 px-2">
            <div className="grid gap-6">
                ...
                <div>
                    <StepHeader title="A entidade transportadora tem histórico de sinistros?" />
                    ...
                    <Check.Radio
                        ...
                        itemList={clainHistories}
                        value={claimHistoryId}
                        setValuesFn={setClaimHistoryId}
                        className="grid sm:grid-cols-2 gap-2 items-start *:w-full gap-y-2 *:text-start *:justify-start"
                    />
                </div>
            </div>
...
function StepFour({
    ...
    franchise: string;
    setFranchise: (e: string) => void;
    franchises: IGetIdAndNameMapperResponse[];
}) {
    return (
        <div className="pt-4 px-2">
            <div className="grid gap-6">
                ...
                <div>
                    <StepHeader title="Locais de Destino e Origem:" />
                    <FlowSelector
                        originLocal={originLocal}
                        destinationLocal={destinationLocal}
                        setOriginLocal={setOriginLocal}
                ...
                <div>
                    <StepHeader title="Franquia - prejuízos indemnizáveis" />
                    <Check.Radio
                        // defaultValue={merchandisesList[0].id}
                        itemList={franchises}
                        value={franchise}
                        setValuesFn={setFranchise}
                        className="grid sm:grid-cols-2 gap-2 items-start *:w-full gap-y-2 *:text-start *:justify-start"
                    />
                </div>
            </div>
